...
export async function createDataWithImage(modelName, data) {
  try {
    const response = await axios.post(`${mainUrl}${modelName}`, data, {
      headers: {
        Accept: "*/*",
        // Axios `Content-Type: multipart/form-data` header-ını avtomatik təyin edir
      },
    });
    return response;
  } catch (error) {
...
export async function updateDataWithImage(modelName, id, data) {
  try {
    const response = await axios.post(`${mainUrl}${modelName}/${id}`, data, {
      headers: {
        Accept: "*/*",
        // Axios `Content-Type: multipart/form-data` header-ını avtomatik təyin edir
        id
...
export async function getAznToUsdRate() {
  ...
  const res = await fetch(
    `https://v6.exchangerate-api.com/v6/${NEXT_PUBLIC_CUR_KEY}/latest/AZN`
  );
